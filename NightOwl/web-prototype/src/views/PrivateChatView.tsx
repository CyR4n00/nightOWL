...
export function PrivateChatView({ friend, onClose }: { friend: { id: string, name: string, status: string }, onClose: () => void }) {
  ...
  const fetchMessages = async (userId: string) => {
    // 🛡️ Sentinel: Validate IDs to prevent PostgREST injection attacks via malicious characters like parentheses or commas.
    const isValidId = (id: string) => /^[a-zA-Z0-9-]+$/.test(id);
    if (!isValidId(userId) || !isValidId(friend.id)) {
