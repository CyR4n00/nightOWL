...
export function PrivateChatView({ friend, onClose }: { friend: { id: string, name: string, status: string }, onClose: () => void }) {
  ...
  const fetchMessages = async (userId: string) => {
    // 🛡️ Sentinel: Validate IDs to prevent PostgREST injection via string interpolation in .or()
    const idRegex = /^[a-zA-Z0-9-]+$/;
    if (!idRegex.test(userId) || !idRegex.test(friend.id)) {
      console.error("Invalid user or friend ID format");
      return;
    }
