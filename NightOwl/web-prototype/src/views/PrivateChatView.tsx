...
export function PrivateChatView({ friend, onClose }: { friend: { id: string, name: string, status: string }, onClose: () => void }) {
  ...
  const fetchMessages = async (userId: string) => {
    // 🛡️ Sentinel: Prevent PostgREST injection by executing two separate queries
    // instead of interpolating variables into a raw .or() query string.
    ...
    if (combinedData.length > 0 || (!sentResponse.error && !receivedResponse.error)) {
