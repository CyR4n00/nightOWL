...
## 2024-06-23 - Unmemoized Randomness in Render
**Learning:** Found that generating random values (e.g., sizes, positions for background bubbles) directly inside the React render body causes severe UI thrashing and re-renders every time the component updates (e.g., on state changes like typing in chat).
**Action:** Always wrap randomized static or visual elements generated during component initialization in a `useMemo` hook to ensure they are calculated once and stable across re-renders.
