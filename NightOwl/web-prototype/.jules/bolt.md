...
## 2024-07-08 - Code-splitting heavy WebRTC SDK dependencies
**Learning:** Found that statically importing the Agora SDK (agora-rtc-sdk-ng) in the main React application bundle adds over 1.5MB to the initial payload, causing significant slowdowns in application startup for all users, even those who do not enter Voice Rooms.
**Action:** Always use `React.lazy()` and `React.Suspense` to dynamically load heavy, route-specific components (like VoiceRoomView) to ensure critical SDKs are only downloaded when the user actually accesses the feature, drastically improving initial time-to-interactive.
