...
## 2024-07-29 - Code Splitting Large Views
**Learning:** Initial page load was slowed down by including `VoiceRoomMainView` in the main chunk. This view, and its underlying components, likely load heavy dependencies (like Agora SDK if imported) which aren't needed until the user switches to the 'voice' tab.
**Action:** Use `React.lazy` and `Suspense` to code-split large route views that aren't visible on the initial render, specifically extracting named exports via `.then(module => ({ default: module.ExportName }))`.
