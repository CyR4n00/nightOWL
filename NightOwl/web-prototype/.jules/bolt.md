...
## 2024-06-25 - React.lazy for large route components
**Learning:** Found that importing heavy route components (like `VoiceRoomView` with `agora-rtc-sdk-ng`) directly causes the main bundle size to explode, leading to Vite build warnings and slow initial load times for users who aren't even visiting those routes.
**Action:** Always use `React.lazy()` and `React.Suspense` to code-split large route components. For named exports, use the pattern `const Comp = lazy(() => import('./Path').then(module => ({ default: module.Comp })))`.
