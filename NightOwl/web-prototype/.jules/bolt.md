...
## 2024-06-25 - Code-splitting large route components
**Learning:** Found that directly importing all views (like VoiceRoomView which could pull in the heavy Agora SDK) blocked the initial load.
**Action:** Always code-split large route components with 'lazy' and 'Suspense' using the '.then(module => ({ default: module.ComponentName }))' pattern for named exports, ensuring they only load when the user navigates to them, significantly reducing the initial bundle size.
