...
## 2024-06-27 - Lazy load heavy code-split components
**Learning:** Initial application load can be significantly delayed by large bundle sizes from dependencies like the Agora SDK.
**Action:** Use `React.lazy()` and `React.Suspense` to code-split large route components like `VoiceRoomView` to prevent blocking the initial application load and to significantly reduce the main JavaScript bundle size.
