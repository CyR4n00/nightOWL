...
## 2025-02-14 - Optimize random deepsea bubble rendering
**Learning:** Initializing randomized arrays inside `useMemo` hooks when length is static and it relies on `Math.random()` creates unnecessary allocations inside component scope even with a static dependency list. Hoisting to the top level provides a notable CPU saving.
**Action:** Always hoist static random generation array values to the module root.
