...
## 2026-06-26 - [Missing Authentication on Edge Functions]
**Vulnerability:** Edge functions meant for internal processes (e.g. pg_cron or Supabase Scheduler tasks) lacked explicit authentication.
**Learning:** Supabase Edge functions are publicly accessible by default unless authorization is enforced within the function logic, making them susceptible to unauthorized invocation if the URL is known.
**Prevention:** Always implement explicit authentication checks (like verifying the `Authorization` header against `SUPABASE_SERVICE_ROLE_KEY` or a custom secret) in Edge Functions intended for scheduled or internal tasks.
