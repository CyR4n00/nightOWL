...
## 2026-07-29 - [Missing Authentication on Edge Functions]
**Vulnerability:** The `daily-cleanup` Supabase Edge Function lacked authentication, allowing any external actor to trigger it.
**Learning:** Edge Functions intended for internal/scheduled tasks (e.g., cron jobs) do not inherently enforce authentication. If not explicitly checked, they are vulnerable to unauthorized execution.
**Prevention:** Always implement explicit authentication checks (like verifying the `Authorization` header against `SUPABASE_SERVICE_ROLE_KEY`) within Edge Function logic when intended for restricted or internal use.
