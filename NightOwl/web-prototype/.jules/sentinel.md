...
**Vulnerability:** Supabase `.or()` query filters used string interpolation directly with user-supplied or external IDs (`.or(\`and(sender_id.eq.${userId},...)\`)`), allowing for PostgREST injection if an ID contains malicious filter operators.
**Learning:** The Supabase SDK does not automatically parameterize variables embedded inside the string argument of advanced filters like `.or()`. This creates a vulnerability similar to SQL injection.
**Prevention:** Strictly validate any external variables (e.g. enforcing alphanumeric/UUID format via regex) before they are interpolated into Supabase string-based filters.
