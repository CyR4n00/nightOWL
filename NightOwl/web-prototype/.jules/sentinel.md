...
## 2026-07-24 - [Missing Authentication on Edge Functions]
**Vulnerability:** Supabase Edge Functions created for background tasks or webhooks (like daily-cleanup) lacked authentication, allowing any user to invoke them.
**Learning:** Supabase Edge Functions do not enforce standard Row Level Security or JWT checks implicitly if invoked via URL directly without using Supabase client wrappers that attach headers. If an endpoint is meant for internal cron tasks, it is publicly exposed by default unless secured.
**Prevention:** Always verify the `Authorization` header against a known secret like `SUPABASE_SERVICE_ROLE_KEY` within the Edge Function code before processing any sensitive logic.
