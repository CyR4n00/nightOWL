...
## 2024-07-30 - Hidden File Upload Inputs Accessibility
**Learning:** Using Tailwind's `hidden` (`display: none`) class on `<input type="file">` elements removes them from the accessibility tree, making them inaccessible to screen readers and keyboard navigation, even if wrapped in a label.
**Action:** Always use `sr-only` for visually hidden file inputs to preserve accessibility, add an `aria-label`, and ensure the wrapping `<label>` has `focus-within` styles to provide a visual indicator for keyboard users.
